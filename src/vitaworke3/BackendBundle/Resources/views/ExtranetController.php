		...
		if ($peticion->getMethod() == 'POST') {
			...
			if ($formulario->isValid()) 
			{
				...
				if ($enviar=='true')
				{
					...
					'BackendBundle:Activitat:email.html.twig',array('usuari' => $client,'activitat'=>$activitatdeldia,'host'=>$host,'calendari'=>$calendari,'accio'=>'crear')
					);
...
public function CalendariActivitatVisualitzarAction($id)
	{
		...
		//$this->get('security.context')->setToken($token);
	 	//$user='Admin'
		//$token = new \Symfony\Component\Security\Core\Authentication\Token\UsernamePasswordToken($user, null, 'main', array("ROLE_ADMIN"));
    	//$this->get('security.context')->setToken($token);
        //$session = $this->get('session');
	    //$session->set('_security_main_',serialize($token));
...
public function CalendariValorarAction($id)
{
		...
		$formulario = $this->createForm(new CalendariValorarType());
				...
				{
					$formulario->bind($peticion);
					$valoracio = $formulario->getData()->get('star1');
					$calendari->setValoracio($valoracio);
					$valorada=new \DateTime('now');
					$calendari->setValorada($valorada);	
					$em->persist($calendari);
					$em->flush();
					return $this->render('BackendBundle:Extranet:portada.html.twig',array('formulario' => $formulario->createView()));
	
				}
				return $this->render('BackendBundle:Extranet:portada.html.twig',array('calendari'=>$calendari, 'formulario' => $formulario->createView()));
