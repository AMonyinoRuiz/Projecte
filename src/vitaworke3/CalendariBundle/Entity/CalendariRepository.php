...
public function enviarmail($calendari,$origen,$contenedor,$diaactivitat)
{
   ...
   if (empty($contingut)){
	...
	if ($origen=='app'){
		$texto = $contenedor->renderView('BackendBundle:Activitat:email.html.twig',array('usuari' => $client,'activitat'=>$activitatdeldia,'host'=>$host,'calendari'=>$calendari,'titol1'=>$titol1,'titol2'=>$titol2,'nick'=>$nick,'contingut'=>$contingut,'accio'=>'crear'));
	}else{		
		$texto = $contenedor->get('twig')->render('BackendBundle:Activitat:email.html.twig',array('usuari' => $client,'activitat'=>$activitatdeldia,'host'=>$host,'calendari'=>$calendari,'titol1'=>$titol1,'titol2'=>$titol2,'nick'=>$nick,'contingut'=>$contingut,'accio'=>'crear'));
	}
...
public function querycalendariplanning($client,$limite=null)
{
	$em = $this->getEntityManager();
	$consulta = $em->createQuery('
	SELECT o.id FROM CalendariBundle:Calendari o where o.Client=:client');
	$consulta->setParameter('client', $client);
	$consulta->getMaxResults(1);
	return $consulta->getResult();
}

public function queryclientcalendarifiltre($idassociat)
  {
	$query='SELECT o FROM ClientBundle:Client o WHERE :idassociat MEMBER OF o.Associat';
    $em = $this->getEntityManager();
    $consulta = $em->createQuery($query);
